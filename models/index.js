...
db.users.belongsToMany(db.books, { through: 'User_Books', timestamps: false });
db.books.belongsToMany(db.users, { through: 'User_Books', timestamps: false });

//define the relationship 1:N between Books and categories
db.categories.hasMany(db.books); // if tutorial is deleted, delete all comments associated with it
db.books.belongsTo(db.categories);

//define the relationship 1:N between Books and Books_Module
db.books.hasMany(db.book_modules); // if tutorial is deleted, delete all comments associated with it
db.book_modules.belongsTo(db.books);

//define the relationship 1:N between Books_Module and Activity
db.book_modules.hasMany(db.activity); // if tutorial is deleted, delete all comments associated with it
db.activity.belongsTo(db.book_modules);


//define the relationship 1:N between Books_Module and Activity
db.activity_type.hasMany(db.activity); // if tutorial is deleted, delete all comments associated with it
db.activity.belongsTo(db.activity_type);

//define the relationship N:M between Users and Books models
db.users.belongsToMany(db.activity, { through: 'User_History', timestamps: false });
db.activity.belongsToMany(db.users, { through: 'User_History', timestamps: false });

//define the relationship N:M between Users and Books models
db.users.belongsToMany(db.achievements, { through: 'User_Achievements', timestamps: false });
db.achievements.belongsToMany(db.users, { through: 'User_Achievements', timestamps: false });

//define the relationship 1:N between Books_Module and Activity
db.levels.hasMany(db.users); // if tutorial is deleted, delete all comments associated with it
db.users.belongsTo(db.levels);

//define the relationship 1:N between Books_Module and Activity
db.users.hasMany(db.notifications); // if tutorial is deleted, delete all comments associated with it
